...
KANBAN_GUIDANCE = (
    ...
    "- **Deliverables.** Scratch workspaces are DELETED when the card "
    "completes. Gate logs and EVIDENCE/DELIVERY/VERDICT files MUST be passed "
    "to `kanban_complete(artifacts=[<absolute paths>])` (top-level param; "
    "paths in `metadata` are NOT uploaded) or attached first with "
    "`kanban_attach`, so they land in `attachments/<id>/` which is not "
    "reaped. Files must exist at completion. The kernel also auto-promotes "
    "root-level evidence-shaped files (`.md`/`.json`/`.log`/`.patch`/`.mjs`/`.py`, "
    "skipping secret-shaped names); uniquely named files outside that set "
    "still need an explicit `artifacts=` entry.\n"
